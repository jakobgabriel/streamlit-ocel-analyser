...
st.set_page_config(page_title="Event Log - Direct Follows Graph Abstraction", layout="wide")

# Function to obtain the DFG abstraction
def get_dfg_abstraction():
    if 'log' not in st.session_state:
        ...
        st.sidebar.header("DFG Abstraction Settings")
        ...
        if st.sidebar.button("Generate DFG Abstraction"):
            dfg_abstraction = pm4py.llm.abstract_dfg(
                log_obj=log,
                max_len=max_len,
...
if 'log' not in st.session_state:
    ...
    log = st.session_state['log']
    st.title("Event Log - Direct Follows Graph Abstraction")
